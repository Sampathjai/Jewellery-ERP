...
import { ShieldCheck, Clock, Lock, Power, RefreshCw, Save, CheckCircle, AlertTriangle, Users, KeyRound } from 'lucide-react';
...
export const UserLoginSettings: React.FC = () => {
  ...
  const [logoutEnabled, setLogoutEnabled] = useState<boolean>(
    db.settings.inactivity_logout_enabled ?? true
  );
  const [timeoutMinutes, setTimeoutMinutes] = useState<number>(
    db.settings.inactivity_timeout_minutes ?? 15
  );
  ...
  const handleSaveSettings = (e: React.FormEvent) => {
    ...
    updatedDb.settings.inactivity_logout_enabled = logoutEnabled;
    updatedDb.settings.inactivity_timeout_minutes = timeoutMinutes;
  ...
  const handleForceLogoutAll = () => {
    if (window.confirm('Are you sure you want to force logout all active staff sessions? All non-admin users will be required to log in again.')) {
      ...
      setTimeout(() => setForceLogoutAlert(null), 5000);
    }
  };
    ...
    <div className="space-y-6 pb-12">
      ...
      {savedSuccess && (
        <div className="rounded-2xl border border-emerald-300 bg-emerald-50 p-4 text-xs font-bold text-emerald-800 dark:border-emerald-800 dark:bg-emerald-950/40 dark:text-emerald-300 flex items-center gap-2 shadow-sm">
          ...
          <span>User Login & Inactivity Settings saved successfully! New session timeouts will apply immediately.</span>
        </div>
      )}
      ...
      <form onSubmit={handleSaveSettings} className="space-y-6 max-w-4xl">
        ...
        <div className="flex justify-end gap-3 pt-2">
          <button
            type="submit"
            className="flex items-center gap-2 rounded-xl bg-gold-500 px-6 py-3 text-xs font-bold text-charcoal-950 shadow-gold hover:bg-gold-600"
          >
            <Save className="h-4 w-4" /> Save Login & Session Settings
          </button>
        </div>
      </form>
    </div>
