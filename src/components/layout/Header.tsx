...
export const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <header className="flex flex-col sm:flex-row sm:h-16 shrink-0 justify-between gap-1.5 sm:gap-2 border-b border-slate-200 bg-white/95 px-2.5 sm:px-6 py-2 sm:py-0 backdrop-blur dark:border-charcoal-800 dark:bg-charcoal-900/95 z-30 w-full max-w-full overflow-hidden pl-[max(0.625rem,env(safe-area-inset-left))] pr-[max(0.625rem,env(safe-area-inset-right))] pt-[max(0.5rem,env(safe-area-inset-top))]">
      {/* Top Bar on Mobile / Left Section on Desktop */}
      <div className="flex items-center justify-between gap-1.5 sm:gap-4 w-full min-w-0 sm:justify-start">
        {/* Mobile-only toggle sidebar & branding */}
        <div className="flex items-center gap-1.5 shrink min-w-0">
          <button
            onClick={onToggleSidebar}
            aria-label="Open navigation menu"
            className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl border border-slate-200 text-slate-600 hover:bg-slate-100 active:bg-slate-200 dark:border-charcoal-800 dark:text-slate-300 dark:hover:bg-charcoal-800 lg:hidden transition-colors"
          >
            <Menu className="h-5 w-5" />
          </button>
          <div className="lg:hidden shrink min-w-0">
        ...
        <div className="hidden sm:flex items-center gap-2 rounded-full border border-amber-200/80 bg-amber-50/80 px-3 py-1.5 text-xs text-amber-900 dark:border-gold-800/40 dark:bg-gold-950/40 dark:text-gold-300 shrink min-w-0 overflow-x-auto whitespace-nowrap shadow-sm scrollbar-none">
          ...
          <span className="font-bold shrink-0">{language === 'ta' ? 'இன்றைய சென்னை விலைகள்' : "Today's Chennai Rates"}:</span>
          {metalRate ? (
            <>
              ...
              <span className="rounded bg-gold-500/20 px-1.5 py-0.5 text-[9px] font-bold uppercase tracking-wider text-amber-900 dark:text-gold-300 border border-gold-300/40 ml-0.5 shrink-0">
                {metalRate.source === 'manual' ? 'Shop Rate' : 'Chennai Rate'}
              </span>
            </>
          ) : (
            <span className="text-slate-400 italic">Chennai Gold Rate: Loading...</span>
          )}
        </div>
        ...
        <div className="flex sm:hidden items-center gap-1 shrink-0">
          <button
            ...
            className="flex h-9 min-w-[34px] items-center justify-center gap-1 rounded-xl border border-gold-400/80 bg-gold-50/90 px-1.5 text-xs font-bold text-charcoal-950 dark:border-gold-700/80 dark:bg-gold-950/60 dark:text-gold-200 hover:bg-gold-100 active:scale-95 transition-all shadow-sm"
          >
            <Globe className="h-3.5 w-3.5 text-gold-600 shrink-0" />
            <span className="text-[11px] font-bold">{language === 'en' ? 'தமிழ்' : 'ENG'}</span>
          </button>
          ...
          <button
            ...
            className="flex h-9 w-9 items-center justify-center rounded-xl border border-slate-200 text-slate-600 hover:bg-slate-100 active:scale-95 dark:border-charcoal-800 dark:text-slate-300 dark:hover:bg-charcoal-800 transition-all shrink-0"
          >
          ...
          <button
            ...
            className="relative flex h-9 w-9 items-center justify-center rounded-xl border border-slate-200 text-slate-600 hover:bg-slate-100 active:scale-95 dark:border-charcoal-800 dark:text-slate-300 dark:hover:bg-charcoal-800 transition-all shrink-0"
          >
          ...
          <div className="relative">
            <button
              ...
              className="flex h-9 items-center gap-1 rounded-xl border border-gold-400/40 bg-gold-50/60 px-1.5 text-xs font-bold text-charcoal-900 dark:border-gold-800/40 dark:bg-gold-950/40 dark:text-gold-200 hover:border-gold-500 active:scale-95 transition-all shrink-0"
            >
              <UserCheck className="h-3.5 w-3.5 text-gold-600 shrink-0" />
              <span className="rounded bg-gold-500 px-1.5 py-0.5 text-[9px] font-extrabold text-charcoal-950 uppercase shrink-0">
                {role}
              </span>
            </button>
